...
First, LP (\ref{eqn:SLC}) is \textcolor{red}{a linear programming problem involving an exponential number of constraints.}
...
Compared with the LP approach in \cite{leastcore2018}, the LRB algorithm is effective since, \textcolor{red}{for those integer linear programming problems having no integrality property,} Lagrangian bounds are sharper than linear relaxation bounds.
...
\textcolor{red}{There are several specific methods, such as the linear relaxation based heuristics, which can generate bounds for $c(S)$. For the sake of consistency and generality, bounds of $c(S)$ in the remainder of this paper are both obtained by the Lagrangian relaxation method.}
...
(ii) for each linear segment of $z_r(\omega)$, its derivative $z'_r(\omega)$ is in the range $\big[ -\frac{v-1}{v}, -\frac{1}{v} \big] \subset \big[-1,0\big]$, \textcolor{red}{where $v$ represents the number of players in $V$, i.e., $|V|$}.
\end{remark}
...
\textcolor{red}{In the sub-gradient method, the initial Lagrangian multipliers vector, $\lambda_0$, is set to be a zero one and the sequence $\{\lambda_k\}$ is generated by recursive equation $\lambda_{k+1}= \lambda_k + t_k (Ax^k - b)$, where $x^k$ is an optimal solution to the Lagrangian problem $Z(\lambda_k) = \min \big\{ cx + \lambda_k (Ax - b): Tx \geq e, x \in \{0,1\}^{n \times 1} \big\}$ and $t_k$ is the recursive step size. The step size used most commonly is
$t_{k}=u_{k}\left(Z^{*}-Z\left(\lambda_{k}\right)\right)}/{\left\|A x^{k}-b\right\|^{2}$, where $u_k$ is determined by setting $u_0 = 2$ and decreasing by halving whenever $Z(\lambda_{k})$ does not change its value after some fixed number of iterations, $\left\| X \right\|$ represents $L_2$-norm of $X$. And $Z^{*}$ is an upper bound on $Z =\max_{\lambda} Z(\lambda)$,
commonly the initial upper bound can be an arbitrary feasible solution for the original integer programming and can be updated whenever finding a new feasible solution during the iteration.}
...
\textcolor{red}{
The conversion is equivalent since for the optimal $z^*$ of LP (6), as long as $\beta(S) = c_\lambda(S) + z^*$ for some $S$, and $-\beta(V) < c_u(V) + \omega$, one can find a new cost allocation $\beta^{'} = \beta - \delta (\delta>0)$ such that $\beta^{'}(S) < c_\lambda(S) + z^*$ and $-\beta^{'}(V) = c_u(V) + \omega$, which conflicts to the optimum of $z^*$.}
