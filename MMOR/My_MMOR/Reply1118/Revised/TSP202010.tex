...
\textcolor{red}{There are several specific methods, such as LP-based ones, which can be used to get the bounds for $c(S)$. For the sake of consistency and generality, the upper and lower bounds of $c(S)$ are obtained by the Lagrangian relaxation method.}
...
$t_{k}=u_{k}\left(Z^{*}-Z_{T}\left(\lambda^{k}\right)\right)}/{\left\|A x^{k}-b\right\|^{2}$, where $u_k$ is determined by setting $u_0 = 2$ and halving $u_k$ whenever $Z_{T}(\lambda)$ does not change its value after some fixed number of iterations, $\left\| \cdot \right\|$ represents 2-norm of $\cdot$. And $Z^{*}$ is an upper bound on $Z_{T}=\max_{\lambda} Z_{T}(\lambda)$
...
% }}
Consequently, $z_r(\omega)$ is the point-wise maximum of a finite set of straight lines, each with a slope of $-\rho_v$. Thus, $z_r(\omega)$ is a piece-wise linear and convex function  $\omega$.
