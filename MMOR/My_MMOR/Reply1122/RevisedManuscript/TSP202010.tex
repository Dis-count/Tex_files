...
\textcolor{red}{In the sub-gradient method, the initial Lagrangian multipliers, $\lambda^0$, are set to a zero vector and the sequence $\{\lambda^k\}$ is generated by $\lambda^{k+1}= \lambda^k + t_k (Ax^k - b)$, where $x^k$ is an optimal solution to the Lagrangian problem $Z_T(\lambda^k)$, $Z_T(\lambda^k) = \min \big\{ cx + \lambda^k (Ax - b): Tx \geq e, x \in \{0,1\}^{n \times 1} \big\}$ and $t_k$ is a positive scalar step size. The step size used most commonly is
...
\textcolor{red}{
This conversion is feasible and the explanation is shown below.
Firstly, LP(4) is to find the minimum value, thus, we just need to limit $c_u(V)- \beta(V)$, which is equal to $\omega$ in (4), not to increase to realize a feasible conversion because $z_{\lambda}(\omega)$ is decreasing with respect to $\omega$ as in Remark $\ref{rem:convex}$. If $c_u(V)- \beta(V)$ can be larger than $\omega$ in a new feasible region, $z_{\lambda}(\omega)$ will be less than the original minimum one in (4), which violates the equivalence of conversion. In this way, $c_u(V)- \beta(V)$ need to be no larger than $\omega$, that is, $c_u(V)- \beta(V) \leq \omega$. After reformulating, we can obtain the second constraint in (6).}
