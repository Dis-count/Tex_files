...
\textcolor{red}{There are several specific methods, such as LP-based ones, which can be used to obtain the upper bound $c_u(V)$.}
...
where the restricted coalition set is $\mathbb{S}' \subset \mathbb{S}$, with a polynomial number of elements, and compute its optimal dual solution $\pi^{*} \in \R^{v+1}$. \textcolor{red}{for example, \{\{1\},\{2\},\ldots, \{v\}\}}
...
We do not mention how to obtain the upper bound.} where $\phi(N)$ is the best social optimum (an upper bound) of the centralized TSP $c_t(N)$, and $\alpha_{\sigma}$ is the best cost allocation (a lower bound) of the optimal cost allocation problem $\alpha_t^*$.
